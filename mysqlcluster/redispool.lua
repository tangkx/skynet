...
function dbpool:addDB(number)
	...
	local count = 0
	for i=1, number do
		local db = redis.connect(self.redisconf)
		if not db then 
			print("redis connect fail")
		else
			if lock(pushDB, self.pool, db, self.lock) == 1 then
				printE("add redisdb is fail")
			else
				printI("add db is success")
				count = count + 1
			end
		end
	end

	printI("add DB is"..count)
end

function dbpool:init(dbconf)

	self.totalNum = dbconf.totalNum or 15
	self.conf = dbconf.redisconf or nil
	...
	for i = 0, self.totalNum do
		...
		if not db then
			...
			if lock(pushDB, self.pool, db, self.lock) == 1 then
				printE("push redisdb to pool fail")
				break
			else
				--printI("push redisdb to pool success")
				count = count + 1
			end
		end
	end
